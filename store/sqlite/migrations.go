...
`)
		...
		&migrate.Migration{
			...
			Up: func(ctx context.Context, exec migrate.Executor) error {
				// A new table, so plain CREATE TABLE is enough; no need for the
				// create-new/copy/drop/rename dance the retention_policies rebuild
				// above needs for an already-populated table.
				_, err := exec.Exec(ctx, `
...
`)
			...
			Down: func(ctx context.Context, exec migrate.Executor) error {
				_, err := exec.Exec(ctx, `DROP TABLE IF EXISTS chronicle_checkpoints;`)
				return err
			},
		},
	)
